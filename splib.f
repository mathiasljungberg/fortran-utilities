...
4     CONTINUE                                                          
      ...
      END                                                               
c
c     These routines below needs the NAG fft function c06ebf 
c    /mathias
C                                                                       
c      SUBROUTINE FCCHGA(N,QZ,CO)                                        
************************************************************************
...
c      DIMENSION QZ(1), CO(0:*)                                          
c      IF(N .EQ. 0) RETURN                                               
c                                                                        
c          PH = 1.57079632679489661923D0                                 
c          R2 = 1.41421356237309515D0                                    
c          DN = DFLOAT(N)                                                
c          CO(0) = QZ(1)/DN                                              
c      IF(N .EQ. 1) RETURN                                               
c                                                                        
...
c          CS = DCOS(AR)                                                 
c          SI = DSIN(AR)                                                 
c          V1 = C*SM*(CO(M)*CS+CO(N-M)*SI)                               
c          V2 = C*SM*SN*(CO(M)*SI-CO(N-M)*CS)                            
c          CO(M) = V1                                                    
c          CO(N-M) = V2                                                  
c          SM = -SM                                                      
c2     CONTINUE                                                          
...
c          AR = PI*DFLOAT(M)/DN                                          
c          SI = .5D0/DSIN(AR)                                            
c          V1 = CO(M)*(1.D0+SI)+CO(N-M)*(1.D0-SI)                        
c          V2 = CO(M)*(1.D0-SI)+CO(N-M)*(1.D0+SI)                        
c          CO(M) = C*SM*V1                                               
c          CO(N-M) = C*SM*SN*V2                                          
c          SM = -SM                                                      
c4     CONTINUE                                                          
...
c      END                                                               
cC                                                                       
c      SUBROUTINE FVCHGL(N,CO,QN)                                        
c*******************************************************************     
c*  COMPUTES USING FFT THE VALUES OF A POLYNOMIAL AT THE CHEBYSHEV       
c*  GAUSS-LOBATTO NODES FROM ITS CHEBYSHEV FOURIER COEFFICIENTS          
c*  N   = THE DEGREE OF THE POLYNOMIAL                                   
c*  CO  = FOURIER COEFFICIENTS OF THE POLYNOMIAL, CO(I), I=0,N           
c*  QN  = VALUES OF THE POLYNOMIAL AT THE NODES, QN(I), I=0,N            
c*******************************************************************     
c      IMPLICIT DOUBLE PRECISION (A-H,O-Z)                               
c      DIMENSION QN(0:*), CO(0:*)                                        
c      IF(N .EQ. 0) RETURN                                               
...
c      DO 1 I=1,N-N2-1                                                   
c          I2 = 2*I                                                      
c          QN(N-I) = CO(I2+1)-CO(I2-1)                                   
c          QN(I) = CO(I2)                                                
c1     CONTINUE                                                          
c      IF (2*N2 .NE. N) THEN                                             
c          QN(N2+1) = 2.D0*CO(N)-CO(N-2)                                 
c      ENDIF                                                             
c                                                                        
...
c          AR = PI*DFLOAT(M)/DN                                          
c          SI = .5D0/DSIN(AR)                                            
c          V1 = QN(M)*(1.D0+SI)+QN(N-M)*(1.D0-SI)                        
c          V2 = QN(M)*(1.D0-SI)+QN(N-M)*(1.D0+SI)                        
c          QN(M) = C*V1                                                  
c          QN(N-M) = C*V2                                                
c4     CONTINUE                                                          
c                                                                        
...
c      END                                                               
cC                                                                       
c      SUBROUTINE FDCHGL(N,QN,CD,DQN)                                    
c*********************************************************************   
c*  COMPUTES USING FFT THE FOURIER COEFFICIENTS AND THE VALUES AT THE    
c*  CHEBYSHEV GAUSS-LOBATTO NODES OF THE DERIVATIVE OF A POLYNOMIAL      
c*  FROM THE VALUES ATTAINED BY THE POLYNOMIAL AT THE NODES              
c*  N   = THE DEGREE OF THE POLYNOMIAL                                   
c*  QN  = VALUES OF THE POLYNOMIAL AT THE NODES, QN(I), I=0,N            
c*  CD  = FOURIER COEFFICIENTS OF THE DERIVATIVE, CD(I), I=0,N           
c*  DQN = VALUES OF THE DERIVATIVE AT THE NODES, DQN(I), I=0,N           
c*********************************************************************   
c      IMPLICIT DOUBLE PRECISION (A-H,O-Z)                               
c      DIMENSION QN(0:*), CD(0:*), DQN(0:*)                              
c      IF(N .EQ. 0) RETURN                                               
c                                                                        
c      CALL FCCHGL(N,QN,DQN)                                             
c          CD(N) = 0.D0                                                  
c          CD(0) = DQN(1)                                                
c      IF(N .EQ. 1) GOTO 2                                               
c                                                                        
c          DN = DFLOAT(N)                                                
c          CD(N-1) = 2.D0*DN*DQN(N)                                      
c      DO 1 K=0,N-2                                                      
c          KR = N-K-2                                                    
c      IF(KR .NE. 0) THEN                                                
c          DK = 2.D0*(DFLOAT(KR)+1.D0)                                   
c          CD(KR) = CD(KR+2)+DK*DQN(KR+1)                                
c      ELSE                                                              
c          CD(0) = .5D0*CD(2)+DQN(1)                                     
c      ENDIF                                                             
c1     CONTINUE                                                          
c                                                                        
cc2     CALL FVCHGL(N,CD,DQN)                                             
c                                                                        
c      RETURN                                                            
c      END                                                               
C                                                                       
